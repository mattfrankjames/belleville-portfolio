...
- **Theming**: colors use `light-dark()`, so the site follows the visitor's light or dark OS setting. To ship light-only, set `color-scheme: light` in `_tokens.css` and in the `<meta name="color-scheme">` tag in `base.njk`.
